...
function peer(other, polite, fail = null) {
  ...
  if (!window.assert_equals) {
    window.assert_equals = (a, b, msg) => a === b || fail(new Error(`${msg} expected ${b} but got ${a}`));
  }

  const log = str => { console.log((polite ? "[POLITE] " : "[IMPOLITE] ") + str); }
  ...
  try {
    ...
    pc.addEventListener("icecandidate", ({candidate}) => send(other, {candidate}));
...
async function setupIframes() {
  ...
  // await setupPoliteIframe();
  // await setupImpoliteIframe();
}
setupIframes();

async function setupIframe(el, polite, r1,g1,b1, r2,g2,b2) {
  const role = polite ? 'Polite' : 'Impolite';
  el.srcdoc =
    `<html\>
  <body\>
  <p\>${role} Peer's iframe</p\>
  <p\>
  ...
  <script\>
    (${peer.toString()})(window.parent.document.getElementById("${polite ? 'impolite' : 'polite'}").contentWindow, false);
  </script\>
  <html\>`;
...
/*
async function setupPoliteIframe() {
  politeIframe.srcdoc =
    `<html\>
  <body\>
  <p\>Polite Peer's iframe</p\>
  <p\>
    <button onclick="(${startLocalVideo.toString()})(0,1,0,0,1,1);"\>Start</button\>
    <button onclick="window.parent.run(window, 'swapTransceivers')"\>Swap Sending Track</button\>
  </p\>
  <p\>
    <video id="localVideo1" autoplay\></video\>
  ...
  <script\>
    (${peer.toString()})(window.parent.document.getElementById("impolite").contentWindow, true);
  </script\>
  <html\>`;
  await new Promise(r => politeIframe.onload = r);
}

async function setupImpoliteIframe() {
  impoliteIframe.srcdoc =
    `<html\>
  <body\>
  <p\>Impolite Peer's iframe</p\>
  <p\>
    <button onclick="(${startLocalVideo.toString()})(1,0,0,1,0,1);"\>Start</button\>
    <button onclick="window.parent.run(window, 'swapTransceivers')"\>Swap Sending Track</button\>
  </p\>
  <p\>
    <video id="localVideo1" autoplay\></video\>
  ...
  <script\>
    (${peer.toString()})(window.parent.document.getElementById("polite").contentWindow, false);
  </script\>
  <html\>`;
  await new Promise(r => impoliteIframe.onload = r);
}
*/
